...
# Состояния
enum State { DIALOGUE, IDLE, CHASE, ATTACK, STUNNED, DEAD, WALK_TO_OBJECT }
...
# Координаты
# Обнови координаты в начале скрипта (отступ от рычага)
var home_pos: Vector3 = Vector3(0.299, 3.116, 30.861)
var lever_pos: Vector3 = Vector3(2.5, 3.116, 23.5) # Поправлено: остановится ПЕРЕД рычагом
...
var health: int = 200:
	set(v):
		health = clamp(v, 0, 200) # Поправил макс хп до 200 как в переменной
...
func _ready():
	...
	if multiplayer.is_server():
		# Даем навигации 0.1 сек прогрузиться и сразу отправляем к рычагу
		get_tree().create_timer(0.1).timeout.connect(go_to_lever)

# --- [ КРИТИЧЕСКИЕ ФУНКЦИИ УРОНА ] ---

# Вызывается из AnimationPlayer (Call Method Track)
func hit_event():
...
func start_opening_speech():
	...
	_play_anim("talk1")
	if multiplayer.is_server():
		get_tree().create_timer(5.0).timeout.connect(_start_tutorial)
...
func _enable_battle_mode():
	...
	_play_anim("talk1")
	if multiplayer.is_server():
		_show_message.rpc("Ну что ж, начнем обучение!")
...
func _end_battle_victory():
	...
	if world_node and world_node.has_method("_sync_ui_element"):
		world_node._sync_ui_element.rpc("label4", true)
		get_tree().create_timer(5.0).timeout.connect(func(): world_node._sync_ui_element.rpc("label4", false))
...
func _sync_death():
	...
	# Отключаем коллизию везде, чтобы труп не мешал ходить
...
func _process_server_logic(delta):
	...
	if is_battle_started:
		...
		return # Выходим, чтобы не выполнять логику ниже во время боя

	# ЕСЛИ БИТВЫ НЕТ, НО МЫ ИДЕМ К РЫЧАГУ
	if current_state == State.WALK_TO_OBJECT:
		...
		if global_position.distance_to(current_move_target) < 0.8:
			if not is_returning_home:
				...
				if world.has_method("_open_gate"):
					world._open_gate.rpc() 
		...
		# Просто стоим/смотрим на игрока до боя
		velocity.x = move_toward(velocity.x, 0, SPEED)
		velocity.z = move_toward(velocity.z, 0, SPEED)
		_find_target()
...
func _handle_animations():
	# Если босс мертв, принудительно оставляем анимацию смерти и выходим
	if is_dead:
		if anim.current_animation != "dying":
			anim.play("dying")
		return
		# Останавливаем плеер на последнем кадре, чтобы он не прыгал в IDLE
		if not anim.is_playing() or anim.current_animation_position >= anim.current_animation_length:
			anim.pause() 
		return

	match current_state:
		State.DIALOGUE: 
			# В режиме диалога анимации запускаются вручную через RPC
			return 
			
		State.WALK_TO_OBJECT: 
			_play_anim("walk")
			
		State.IDLE:
			if is_battle_started or is_transitioning_to_battle:
				_play_anim("idle-with-sword")
			else:
				_play_anim("idle")
				
		State.CHASE:
			_play_anim("run-with-sword")
			
		State.STUNNED:
			_play_anim("impact")
			
		State.ATTACK:
			# Запускаем одну из случайных атак, если сейчас не проигрывается атака
			if not anim.current_animation.begins_with("attack"):
				var attacks = ["attack1", "attack2", "attack3"]
				_play_anim(attacks[randi() % attacks.size()])
...
func _on_anim_finished(anim_name: String):
	...
	if anim_name == "talk1" and is_transitioning_to_battle:
		...
		get_tree().create_timer(5.0).timeout.connect(func():
			if not is_battle_started and is_transitioning_to_battle: _start_actual_combat.rpc()
		)
...
func _set_sword_visibility(is_visible: bool):
	...
	if sword_node: sword_node.visible = is_visible
...
func take_damage(amount: int):
	if is_dead or not multiplayer.is_server(): return
...
func _die():
	if is_dead: return # Чтобы не вызывалось дважды
	_kill_boss_instantly.rpc()
	is_dead = true
	current_state = State.DEAD
	
	# Отключаем физику, чтобы игроки не спотыкались о труп
	$CollisionShape3D.set_deferred("disabled", true)
	
	# Проигрываем анимацию смерти через RPC на всех клиентах
...
func _kill_boss_instantly():
	...
	if anim:
		anim.play("dying")
		# КОСТЫЛЬ: Если это клиент, ждем время падения и вырубаем аниматор наглухо
		var death_anim_length = anim.get_animation("dying").length
		get_tree().create_timer(death_anim_length - 0.1).timeout.connect(func():
			anim.stop(true) # Остановить и оставить в текущей позе
			set_physics_process(false) # Больше никакой логики движения
			set_process(false)         # Больше никакого обновления
		)
...
func _handle_attack_dash():
	...
	# Если первая половина анимации - делаем рывок к цели
	if time_in_anim < 0.5: 
		if is_instance_valid(target):
