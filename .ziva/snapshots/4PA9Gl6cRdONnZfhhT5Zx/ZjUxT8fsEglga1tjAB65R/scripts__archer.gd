...
func _physics_process(delta):
	...
	# Если мы в процессе переката, форсируем движение
	if is_rolling:
...
func _perform_roll():
	...
	# Направление переката - вперед относительно модели
	var roll_dir = -model.global_transform.basis.z 
	roll_velocity = roll_dir * ROLL_SPEED
...
func _perform_dash(dir_name: String):
	...
	# Направление относительно камеры (теперь исправлено: Forward = -Z)
	match dir_name:
		"up": 
