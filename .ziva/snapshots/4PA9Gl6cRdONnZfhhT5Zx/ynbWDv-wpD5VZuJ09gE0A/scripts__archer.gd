...
func _physics_process(delta):
	...
	if is_rolling:
		...
		if roll_timer <= 0:
			is_rolling = false
			# Даем импульс в конце, чтобы не замирать мгновенно
			velocity.x = forced_velocity.x * 0.5
			velocity.z = forced_velocity.z * 0.5
		else:
	...
	if is_dashing:
		...
		if dash_timer <= 0:
			is_dashing = false
			velocity.x = forced_velocity.x * 0.3
			velocity.z = forced_velocity.z * 0.3
		else:
			velocity.x = forced_velocity.x
			velocity.z = forced_velocity.z
			_apply_gravity(delta)
	...
	if is_moving:
		...
		_stop_movement(delta)
		if is_crouching:
			_update_animation("idle", 0.5) # В присяде на месте просто идл
		else:
...
func _stop_movement(delta):
	# Сделаем торможение более плавным, чтобы не было "мгновенной" остановки
...
func _perform_roll():
	...
	_update_animation("somersault", 1.2, 0.1) 
	...
	if move_dir != Vector3.ZERO:
		...
		# Если не нажимаем кнопки, катимся вперед куда смотрит модель
		roll_dir = -model.global_transform.basis.z
		roll_dir.y = 0
...
func _perform_dash(action: String):
	...
	# Поворачиваем модель в сторону рывка
	if model:
