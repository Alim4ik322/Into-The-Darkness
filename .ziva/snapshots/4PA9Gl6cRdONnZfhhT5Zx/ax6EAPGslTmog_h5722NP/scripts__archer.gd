...
const DASH_SPEED = 22.0 # Быстрее ролла
const GRAVITY = 30.0
...
func _handle_normal_state(delta):
	...
	# Проверка на кувырок (Alt)
	if Input.is_key_pressed(KEY_ALT) and is_on_floor() and stamina_bar.value >= 15:
		_perform_roll()
		return
...
func _perform_dash(action: String):
	...
	dash_cooldown = 0.4
	stamina_bar.value -= 10 # Уменьшил стоимость до 10
