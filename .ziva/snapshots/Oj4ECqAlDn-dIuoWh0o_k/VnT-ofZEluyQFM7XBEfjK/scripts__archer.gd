...
func _physics_process(delta):
	...
	# Статы
	if hungry_bar: hungry_bar.value -= 0.7 * delta
	if water_bar: water_bar.value -= 0.3 * delta
	if mp_bar: mp_bar.value = min(100, mp_bar.value + 0.15)
	...
	if Input.is_key_pressed(KEY_ALT) and is_on_floor() and not is_rolling and not is_dashing and (stamina_bar.value if stamina_bar else 100) >= 25:
		_perform_roll()

	# Процесс переката или рывка
	...
	if is_crouching:
		...
		if stamina_bar: stamina_bar.value = min(100, stamina_bar.value + 6 * delta)
	elif is_running:
		target_speed = RUN_SPEED
		if stamina_bar: stamina_bar.value = max(0, stamina_bar.value - 18 * delta)
	else:
		target_speed = WALK_SPEED
		if stamina_bar: stamina_bar.value = min(100, stamina_bar.value + 12 * delta)

	if is_moving:
		# Плавное ускорение
		velocity.x = lerp(velocity.x, move_dir.x * target_speed, delta * 10.0)
		velocity.z = lerp(velocity.z, move_dir.z * target_speed, delta * 10.0)
		...
		if is_running:
			...
			_update_animation("crouch forward") # Обычная ходьба
	else:
		_stop_movement(delta, 15.0)
		if is_crouching:
...
func _perform_roll():
	is_rolling = true
	if stamina_bar: stamina_bar.value -= 25
...
func _perform_dash(dir_name: String):
	if is_dashing or is_rolling or dash_cooldown > 0 or (stamina_bar.value if stamina_bar else 100) < 20: return
	is_dashing = true
...
func _stop_movement(delta: float, friction: float):
...
func _update_animation(anim_name: String):
	...
	for lib_name in all_anims:
		...
		for a in lib.get_animation_list():
			var full_name = a.to_lower()
			if full_name == anim_name.to_lower() or (anim_name.length() > 3 and full_name.contains(anim_name.to_lower())):
...
func _physics_process(delta):
	...
	# Статы (как у воина)
	hungry_bar.value -= 0.8 * delta
	water_bar.value -= 0.4 * delta
	...
	# 2. Перекат (Кнопка Alt)
	if Input.is_key_pressed(KEY_ALT) and is_on_floor() and not is_rolling and not is_dashing and stamina_bar.value >= 20:
		_perform_roll()
	...
	var is_moving = move_dir != Vector3.ZERO
	var is_running = Input.is_action_pressed("Shift") and is_moving and not is_crouching and stamina_bar.value > 0
	...
	if is_crouching:
		...
		target_speed = RUN_SPEED
		stamina_bar.value = max(0, stamina_bar.value - 15 * delta)
	else:
		target_speed = WALK_SPEED
...
# --- Логика Рывка (Dash) ---
func _perform_dash(dir_name: String):
	if is_dashing or is_rolling or dash_cooldown > 0 or stamina_bar.value < 15: return
	is_dashing = true
