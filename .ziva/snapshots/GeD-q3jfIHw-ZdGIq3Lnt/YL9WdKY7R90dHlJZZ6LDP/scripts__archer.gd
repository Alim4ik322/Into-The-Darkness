...
func _ready():
	...
	config.add_property("Archer_model:rotation")
	config.add_property("Archer_model/AnimationPlayer:current_animation")
	sync.replication_config = config
	add_child(sync)

	if not is_multiplayer_authority():
		# Отключаем камеру для чужих игроков. Пытаемся найти по разным путям.
		var camera = get_node_or_null("CameraPivot/CameraVerticalPivot/CameraZoomPivot/Camera3D")
...
func _handle_normal_state(delta):
	...
	# Проверка на кувырок (убрано из physics_process, теперь в _input)
