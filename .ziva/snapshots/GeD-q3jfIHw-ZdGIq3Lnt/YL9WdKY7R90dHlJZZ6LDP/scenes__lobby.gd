...
@onready var p1_name_label = $CanvasLayer/Player1_Name
@onready var p1_class_label = $CanvasLayer/Player1_class
@onready var p_count_label = $CanvasLayer/PlayerCount
...
func _ready() -> void:
	...
	$CanvasLayer.add_child(p2_name_label)
	$CanvasLayer.add_child(p2_class_label)
	
	# Сдвигаем вниз на 80 пикселей (примерно)
	p2_name_label.position.y += 80
	p2_class_label.position.y += 80
	...
	if Global.is_host:
		...
		# Клиент просит данные у хоста
		rpc_id(1, "request_player_list", multiplayer.get_unique_id())
