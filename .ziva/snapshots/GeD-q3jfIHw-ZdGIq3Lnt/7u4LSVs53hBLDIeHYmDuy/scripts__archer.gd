...
func _physics_process(delta):
	...
	if direction:
		...
		$Archer_model.rotation.y = lerp_angle($Archer_model.rotation.y, atan2(direction.x, direction.z), delta * 10)
	else:
...
func _ready():
	print("[DEBUG] Archer _ready. Name: ", name, ", Unique ID: ", multiplayer.get_unique_id())
	
	# СУПЕР ВАЖНО: Устанавливаем власть на клиенте по имени узла (ID)
	if name.is_valid_int():
		var id = name.to_int()
		set_multiplayer_authority(id)
		print("[DEBUG] Archer node ID ", name, " authority set to ", id)
	
	var is_local = is_multiplayer_authority()
	print("[DEBUG] Am I (ID ", multiplayer.get_unique_id(), ") authority for node ", name, "? ", is_local)
	...
	if is_local:
		print("[DEBUG] Archer is LOCAL! Activating camera and UI.")
		if camera:
			camera.make_current()
			camera.current = true
		if has_node("UI"):
		...
		print("[DEBUG] Archer is REMOTE. Deactivating camera and UI.")
		if camera:
			camera.current = false
			camera.queue_free() # Удаляем чужую камеру
		if has_node("UI"):
			$UI.hide()
			$UI.queue_free() # Удаляем чужой интерфейс
		set_physics_process(false)
		set_process_input(false)
