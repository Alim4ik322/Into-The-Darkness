...
func _ready():
	...
	if name.is_valid_int():
		...
		print("[DEBUG] Authority set to: ", id, " for node ", name)
	...
	if is_multiplayer_authority():
		print("[DEBUG] Archer is authority! Activating camera and UI.")
		if camera:
			camera.make_current()
			camera.current = true
		else:
			print("[DEBUG] ERROR: Camera3D NOT FOUND in Archer!")
		if has_node("UI"):
			$UI.show()
	else:
		print("[DEBUG] Archer is NOT authority. Deactivating camera and UI.")
		if camera:
