...
func _ready():
	print("[DEBUG] Player _ready. Name: ", name, ", Unique ID: ", multiplayer.get_unique_id())
	
	# Даем один кадр на синхронизацию имен и ID
	await get_tree().process_frame
	
	# Имя ноды - это ID игрока.
	if name.is_valid_int():
		var id = name.to_int()
		set_multiplayer_authority(id)
		print("[DEBUG] Authority set to: ", id, " for node ", name, ". Am I authority? ", is_multiplayer_authority())
		if has_node("MultiplayerSynchronizer"):
			$MultiplayerSynchronizer.set_multiplayer_authority(id)
	
	var camera = find_child("Camera3D", true)
	if is_multiplayer_authority():
		print("[DEBUG] Activating camera for LOCAL player ", name)
		if camera:
			camera.make_current()
			camera.current = true
		else:
			# Если камеры нет, пробуем создать временную, чтобы не было серого экрана
			print("[DEBUG] ERROR: Camera3D NOT FOUND. Creating fallback camera.")
			var fallback_cam = Camera3D.new()
			fallback_cam.position = Vector3(0, 5, 5)
			fallback_cam.look_at(Vector3.ZERO)
		...
		print("[DEBUG] Deactivating camera for REMOTE player ", name)
		if camera:
