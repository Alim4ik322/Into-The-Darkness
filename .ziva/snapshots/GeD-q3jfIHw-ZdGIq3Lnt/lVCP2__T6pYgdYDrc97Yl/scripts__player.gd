...
func _physics_process(delta):
	...
	if direction:
		...
		$Visual.rotation.y = lerp_angle($Visual.rotation.y, atan2(direction.x, direction.z), delta * 10)
	else:
...
func _ready():
	...
	# Устанавливаем власть по имени узла (ID)
	if name.is_valid_int():
		var id = name.to_int()
		set_multiplayer_authority(id)
		print("[DEBUG] Player node ID ", name, " authority set to ", id)
	...
	if is_local:
		print("[DEBUG] Player is LOCAL! Activating camera and UI.")
		if camera:
			camera.make_current()
			camera.current = true
		if has_node("UI"):
		...
		print("[DEBUG] Player is REMOTE. Deactivating camera and UI.")
		if camera:
